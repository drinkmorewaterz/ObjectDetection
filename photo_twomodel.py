...
def box_label(image, box, label='', color=(128, 128, 128), txt_color=(255, 255, 255)):
  lw = max(round(sum(image.shape) / 2 * 0.003), 1) // 2
  p1, p2 = (int(box[0]), int(box[1])), (int(box[2]), int(box[3]))
  cv2.rectangle(image, p1, p2, color, thickness=lw, lineType=cv2.LINE_AA)
  if label:
    tf = max(lw - 1, 1) // 2 # font thickness
...
resultrail = modelrail.predict(source=img, save=True, hide_labels=True, hide_conf=True, line_thickness=1, imgsz=1280)
...
if resultrail[0].masks:
    ...
    rail_masks = cv2.addWeighted(img, 1, black_mask.astype('uint8'), 0.1, 0)
    cv2.imwrite(r'C:\Users\monke\Desktop\PYYYY\YOLO888\runs\segment\predict\image0.jpg', rail_masks.astype('uint8'))
    rail_image = cv2.imread(r'C:\Users\monke\Desktop\PYYYY\YOLO888\runs\segment\predict\image0.jpg')
results = model.predict(source=rail_image, imgsz=1280, classes=[0,2,6,9])
